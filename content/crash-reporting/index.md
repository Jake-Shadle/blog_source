...
Even though Rust has absolutely delivered on its promise of avoiding entire classes of bugs and crashes (mostly at compile time!) that plague non-memory safe languages like C and C++, the truth is that crashes still happen in Rust projects.

One of the biggest sources for crashes in a Rust project is that, even if a vast majority of code is in Rust, it's almost certain you have at least a few dependencies with code written in C, C++, or asm, not even counting the giant dependency of your OS if you are targeting a non-embedded environment. And while the Rust ecosystem is [slowly](https://github.com/Byron/gitoxide) [rewriting](https://github.com/DoumanAsh/xxhash-rust) [C libraries](https://github.com/KillingSpark/zstd-rs) in Rust, this is a Sysiphean task.
...
So this all seems fairly complicated, why could we not just use Breakpad from Rust? Well, we [did](https://github.com/EmbarkStudios/sentry-contrib-rust/tree/main/breakpad-handler) for a while! And it was "fine".

However, I wanted to transition to use [`x86_64-unknown-linux-musl`](https://musl.libc.org/) on all of our services so that we could get smaller container images for faster deploys. It turned out that Breakpad was our sole C++ dependency for our services (well, most of them, one depends on...[physx](https://github.com/EmbarkStudios/physx-rs) :cry:), which is annoying when targeting musl. It's a solvable problem, but apparently (ok, this happens often for me) I hit it on day that made me [annoyed enough](https://perlbuzz.com/2008/02/15/annoyancedriven_development/) to start porting the Linux code to Rust so that we could at least get rid of the Breakpad dependency for that one target, at least initially.
...
Rather than going the Breakpad/Crashpad route of having a single large project to integrate, perhaps [providing basic control](https://github.com/google/breakpad/blob/0808030bee8bc88a34675cd1dd83b965a2249a08/src/client/linux/handler/exception_handler.h#L76-L114) over how crash handling is done, provide several crates focused on specific aspects in this area that can be used together, or independently. For example, you may want to catch crashes when they occur, but instead of creating a minidump, throw up a dialog saying you are very sorry before your program exits. Or maybe (like us) you want to report minidumps to [Sentry](https://sentry.io) and include not only the minidump, but also the normal event data such as [breadcrumbs](https://docs.sentry.io/product/issues/issue-details/breadcrumb) and [tags](https://docs.sentry.io/product/issues/issue-details/#tags).
...
I had initially started down the path of supporting in-process dumping since we had been using that due to convenience up that point, and frankly, the code was kind of [fun](https://github.com/EmbarkStudios/sentry-contrib-rust/commit/7fedcbffd2230f8aaee41d35622ebf29221994d7) to write (well [mostly](https://github.com/EmbarkStudios/sentry-contrib-rust/commit/20c2883478818e341170b0cfc49141d203783dba#diff-281d2fdeea972facd303c135135513ec97b57d43ff8d5b1f244f85f9a0efef83R111-R113), Rust still doesn't have [stable](https://github.com/rust-lang/rust/issues/32838) [Allocators](https://github.com/EmbarkStudios/sentry-contrib-rust/commit/07373b7667764d58904ea981f80734743d6b6d85) :sad:), however [reason](https://github.com/luser/rust-test-assembler/issues/12#issuecomment-963059409) prevailed and I decided to go all in on out-of-process dumping. Though this decision does have implications I'll touch on at the end.
...
The first component that we need is, well, a way to actually know when a crash has occurred! Each platform has its own special way to inform users of crashes or other exceptional events.

#### Goals

* Catch all of the relevant crash events for the supported platforms
* Allow users to raise their own crash events to take advantage of the same crash context and handling mechanisms (eg an application might want to signal that it has entered a "hung" state and dump its program state for analysis upstream)
* Run user code in the event of a crash, supplying the crash context. This is the most important difference from the Breakpad/Crashpad handlers, which not only catches crash events, but then pass the crash context onto the code/process to write (and maybe send!) the minidump, we simply invoke a user supplied callback. You get to do what you want! (but please be reasonable)
...
Breakpad also registers two other error handlers exclusive to Windows, which are [`_set_invalid_parameter_handler`](https://docs.microsoft.com/en-us/cpp/c-runtime-library/reference/set-invalid-parameter-handler-set-thread-local-invalid-parameter-handler?view=msvc-170), invoked when supplying invalid arguments to CRT (libc) functions eg. `printf(nullptr);`, and [`_purecall`](https://docs.microsoft.com/en-us/cpp/c-runtime-library/reference/purecall?view=msvc-170), called when C++ code tries to invoke a pure virtual function. The effort to support these was minimal so I ported them as well, even though Crashpad seemed to not think they were useful enough to port.
...
Now, to be clear, before I started porting the MacOS Breakpad code to Rust, I had managed to successfully dodge needing to know any Mac/Apple specifics because I've worked in traditional (non-mobile) game development up to this point in my career, and never worked on a project that targeted and Apple device up until the project I work on now. I do manage our Apple CI machines, so I know that XCode fucking sucks, homebrew is terrible, and Apple's complete lack of support for virtualization would be laughable if it wasn't such a giant headache for literally every project that wants to have basic automation for Apple targets.
...
Now, at this moment, [Apple's market capitalization](https://companiesmarketcap.com/apple/marketcap/) is **$2.415 Trillion** so obviously this is not due to lack of resources, but rather will. But let's give Apple the benefit of the doubt and assume that they don't want use to use these APIs because they're "internal", something like the undocumented [`Nt*`](http://undocumented.ntinternals.net/) functions in Windows, which Microsoft purposefully doesn't provide public documentation or support for so that they can change or remove them whenever they want, while still preserving their infamous backwards compatibility.
...
As stated before, the focus, at least for this initial implementation pass, is on out-of-process crash handling. That means, if we want to create a minidump, we need to communicate the crash context from the crashing process to a different one that can take care of those details.
...
In Breakpad, the IPC code is used for exactly 1 thing, the client (crashing process) requesting a minidump be generated by the server (monitor) process. This is the simplest approach, but is, for what we (and, I'm assuming, others) want out of a crash reporting mechanism, too simple. To reiterate, we use [Sentry](https://sentry.io) as our error reporting solution. While it supports just sending minidumps and does the handling of them by symbolicating all the thread callstacks and pulling out eg. system information, it has a _much_ richer set of details that we could provide. But as stated previously, when a crash has occurred and we want to create a minidump for it in another process, we (probably) also want to perform the transmission of that minidump (if the user wants to) in the same process, so that the crashing process itself does as little work as possible to avoid potentially losing crashes.

Let's take a single example of richer information that we would want to send along with a crash, [breadcrumbs](https://docs.sentry.io/product/issues/issue-details/breadcrumbs/). Breadcrumbs are essentially just the last `n` trace events, and can be incredibly useful for diagnosing issues. For example, our primary project is a frame-based application, which could mean that a crash's root cause could have been indicated by a (seemingly) innocuous warning message 1 or more frames previously than the one in which the crash actually occurred.

Our scenario led me to believe that most projects that might want to integrate crash reporting would want some way to do the same thing, sending application specific information to the crash monitor process so that if a crash occurs, the minidump can be accompanied by richer and more detailed information.

#### IPC implementation

Breakpad code uses platform-specific IPC methods, so Linux uses [Unix Domain Sockets](https://en.wikipedia.org/wiki/Unix_domain_socket), Windows uses [Named Pipes](https://docs.microsoft.com/en-us/windows/win32/ipc/named-pipes), and MacOS uses (ugh) Mach ports. I started the port by targeting Linux so naturally just kept the UDS method, though slightly modified to use abstract addresses via the [uds](https://docs.rs/uds/latest/uds/) crate, which was all nice and easy.

Then came Windows.

If you are _only_ targeting Windows, named pipes are, I suppose, an ok way to do IPC. However, if you target anything else, they make sharing code between the targets impossible due their _wildly_ different API compared to sockets or even files. I did an initial pass at supporting them, but there was a bug somewhere and they weren't working correctly. As I was already annoyed that the code was so different from the nice and simple UDS version, I decided to go an alternate route and just...use [Unix Domain Sockets](https://devblogs.microsoft.com/commandline/af_unix-comes-to-windows/). Unfortunately, Windows doesn't support ancillary data, nor abstract addresses like Linux, which means we need to create an actual file on disk and manually clean it up later, but it does mean we can have a much simpler implementation that shares a bulk of the same code. It does come with the caveat that we've now limited support to [Windows 10.17063+](https://blogs.windows.com/windows-insider/2017/12/19/announcing-windows-10-insider-preview-build-17063-pc/), but that's over 4 years old at this point.
...
Following this nice win on Windows, I naively assumed that MacOS would be even easier, but as we've learned, MacOS is a giant pain the ass, and it doesn't disappoint here either. I had UDS working on MacOS, and was catching crashes and dumping them successfully in tests, and all was well. I integrated my changes into our project and emitted a few crash dumps to Sentry, but noticed the crash reports were...not good.
...
Now, people who have knowledge about MacOS are probably thinking, "you fucking idiot, of course that's not how `mach_task_self` works!", but in my defense, Apple sucks, as, "fun fact", `mach_task_self` is not even listed on [Apple's "documentation"](https://developer.apple.com/search/?q=mach_task_self), not that it matters, since we all know they wouldn't have actually had any documentation. I needed to find this random snippet from [Mac OS X Internals](https://books.google.se/books?id=K8vUkpOXhN4C&pg=PA577&lpg=PA577&dq=mach_task_self&source=bl&ots=ONmhYW_s1D&sig=ACfU3U30VoOBwZ1ILTEzVBChP9BMgTsgJw&hl=en&sa=X&ved=2ahUKEwj_zpSulOv3AhVjkosKHSPLAqoQ6AF6BAgfEAM#v=onepage&q=mach_task_self&f=false) to see that I had fundamentally misunderstood the API.
...
So it turns out my simplification to completely replace the Mach ports IPC with UDS was fundamentally flawed. The Mach IPC code is not only doing transmission, it's also checking permissions and, most importantly, converting port identifiers to be...actually useful when sent to another process. While I _think_ it is probably possible to do all of this in userspace, I was too irritated to want to spend more time on this figuring out stupid issues that would be easily avoided if Apple weren't (from my perspective) spiteful bastards, so I just opted for the rather ugly solution of

<p align="center"><img src="https://media.giphy.com/media/3o85xIO33l7RlmLR4I/giphy.gif"/></p>

This means that any user messages are sent via the regular socket the same as Linux and Windows, but when sending a crash context, we fallback to using `mach_msg` (why did Apple feel that using a single function for both sending and receiving was a good idea...). This makes the code significantly uglier, but it does work!

<p align="center"><img src="good_macos_crash.png"/></p>

I _believe_ it will be possible to remove this ugliness in the future after my tolerance for Apple's bullshit has increased (or some kind person who knows MacOS send a PR!), but for now, it works.
...
Minidumps are kind of a general container format, with multiple different streams of information that are (mostly) isolated from each other. Essentially, what each non-Windows platform needs to do to fill out information in a minidump is to inspect the state of the process, similarly to a debugger. For Linux this is mostly done with [`ptrace`](https://man7.org/linux/man-pages/man2/ptrace.2.html), and for MacOS it's _sigh_ a bunch of undocumented APIs. Luckily the Breakpad authors have already done the hard work of writing a C++ implementation that we can mostly directly port from.
...
* `cv_record` - The [debug identifier](https://docs.rs/debugid/latest/debugid/struct.DebugId.html) for the module. This is how a minidump processor can retrieve the debug symbols associated with a module to symbolicate an instruction address to get eg file, function, and line information.
...
This is the stream that contains the actual high level metadata around the crash itself, like the signal, exception code, etc.
...
This stream was added by Breakpad to make processing minidumps easier, it just contains the thread ids of the thread that the crash occurred in, and thread id of the thread that handled the crash. This is so that processors can de-prioritize (or even ignore) the handler thread to simplify the crash report.

This is really only used by MacOS as the handler thread is always distinct from the crashing thread, but can also be useful on Windows if you happen to be doing in-process crash dumping.
...
The Linux implementation is far more fleshed out, both in the new Rust code and original Breakpad code, than the MacOS one, since it's so much easier to retrieve information and have _some_ documentation or existing code to work with than MacOS, but this is definitely an area where I think the Rust crate can improve on top of the original Breakpad implementation to provide more information. If only MacOS wasn't so irritating.
...
That being said we've already integrated these crates into our main Rust project at Embark as a way for me to dogfood the implementation and it's already better than the Breakpad solution we were using before, as it is mostly on par in terms of implementation, but with 100% less C++! So I guess I would _tentatively_ say that you can start trying to use one or more crates in this project if you want to handle crashes and target one or more of the currently supported targets, but probably not in a production environment just yet.

One interesting question for games projects would be support for crash handling on consoles. Unfortunately, this is not going to happen anytime soon. For one, the chosen method of out-of-process crash handling makes this a non-starter on consoles, as, for starters, process creation is not allowed for user applications. While work could be done to make in-process minidump generation work, with the caveat that it would always be less reliable than out-of-process, the sad fact is that there's no point in doing any of that work as, for now, at least 1 major console doesn't even allow Rust binaries. But maybe someday?
